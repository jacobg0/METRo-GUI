...
# create a sub window where the user can select a file to convert from CSV to xml
def csv_xml():
    win2 =Toplevel()
    win2.title('Convert CSV to XML')
    win2.geometry("400x200")




    create_label= Label(win2, text="Using the buttons below, selct the file to convert and the output file", fg="black", font=(None, 10))
    create_label.grid(row=0, column=2)
    ...
    def inputfile():
        ...
        csvfile = tkFileDialog.askopenfilename(filetypes=(("All files", "*.*"), ("XML", "*.xml")))

    btn_input = Button(win2, text= "select the file to input", command =inputfile )
    ...
    #def lift_win1():
    #    win2.lift(aboveThis=root)

    #btn_lift = Button(win2, text="Lift win1", command=lift_win1)
   # btn_lift.pack(padx=30, pady=5)
       ...
       # self.button = button(root, text = "Select your file that you want to convert")
...
#filemenu.add_command(label="New XML file", command=NewFile())
#command=csv_xml)
#filemenu.add_command(label="Open", command=OpenFile)
filemenu.add_command(label="Convert CSV to XML", command=csv_xml)
